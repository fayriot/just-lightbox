...
export class JustLightboxGesturesDirective {
    ...
    handleGesture(event: any) {
        if (this.options.gesturesEnabled && getUserAgent() === UserAgentEnum.Ios) {
    ...
    @HostListener('touchmove', ['$event']) handleTouchMove(event: any) {
        if (this.options.gesturesEnabled && getUserAgent() === UserAgentEnum.Android) {
    ...
    @HostListener('wheel', ['$event']) handleWheel(event: any) {
        if (this.options.wheelEnabled) {
            ...
            if (event.ctrlKey) {
                const s = Math.exp(-event.deltaY / (this.options.wheelSensitivity.zoom * 100));
                this.scale *= s;
            } else {
                const direction = event.checked ? 1 : -1;

                this.tx += (event.deltaX / this.options.wheelSensitivity.pan) * direction;
                this.ty += (event.deltaY / this.options.wheelSensitivity.pan) * direction;
            }
    ...
    private touchMove(event) {
        if (event.touches.length === 2) {
            ...
            this.scale = this.getDistance(event.touches) / this.getDistance(this.initialTouches);
    ...
    private getMidpoint(touches): {x; y} {
        return {
